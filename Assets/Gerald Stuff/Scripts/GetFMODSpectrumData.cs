...
{
    [FMODUnity.EventRef] public string _heartEvent = null;
    ...
    {
        _event = FMODUnity.RuntimeManager.CreateInstance(_heartEvent);
        ...
        _dsp.setParameterInt((int)FMOD.DSP_FFT.WINDOWSIZE, (int)_windowSize * 2);
    ...
    private void GetSpectrumData()
    {
        ...
        {
            for(int b = 0; b < _windowSize; b++)
            {
                float _totalChannelData = 0f;
                for (int c = 0; c < _fftparam.numchannels; c++)
                    _totalChannelData += _fftparam.spectrum[c][b];
                _samples[b] = _totalChannelData / _fftparam.numchannels;
               
            }
            //Debug.Log("working with:" + fftparam.numchannels + "channels here mother FUCKERS!");
        }
    }
}
